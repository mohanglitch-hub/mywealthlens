"""
Migration Script — Document Vault client-side encryption (Sep 2026)
=========================================================================
New columns:

  user table:
    - encryption_salt        VARCHAR(64)   -- base64, NOT secret
    - encryption_verifier    TEXT          -- ciphertext, NOT secret
    - encryption_verifier_iv VARCHAR(64)   -- base64, NOT secret
    - encryption_enabled_at  DATETIME

  wealth_document table:
...
Run from project root: py migrate_add_encryption.py
"""
...
def run_migration():
    with app.app_context():
        print("=" * 60)
        print("Document Vault client-side encryption")
        print("=" * 60)

        with db.engine.connect() as conn:
            _add_columns(conn, "user", [
                ("encryption_salt",        "VARCHAR(64)"),
                ("encryption_verifier",    "TEXT"),
                ("encryption_verifier_iv", "VARCHAR(64)"),
                ("encryption_enabled_at",  "DATETIME"),
            ])
            _add_columns(conn, "wealth_document", [
                ("iv",           "VARCHAR(64)"),
                ("is_encrypted", "BOOLEAN DEFAULT 0"),
            ])
